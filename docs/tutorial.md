...
First we create a new root directory. In this directory we create the depot directory, where the artifacts of the deployments will be stored and fetched later. In an instance directory we create a subfolder for each target environment. In our case this is the build environment, which is the productive environment for this tutorial. Furthermore we create the actual working folder called sourcen. This is the actual development folder. This folder will be versioned via Git.
...
$ mkdir _depot && mkdir -p instances/prod && mkdir sources
...
# add submodul
sources[main]$: git submodule add https://github.com/MaikMichel/dbFlow.git .dbFlow
...
```bash
# add current changes
sources[main]$: git add .

# do the commit
sources[main]$: git commit -m "Inital commit, with dbFlow add as submodule"

# create new branch and jump right in
sources[main]$: git checkout -b develop

```

## 02. Setup dbFlow

Now that we have the directory and git set up, we can configure dbFlow. For our application and just for demo purposes a SingleSchema is completely sufficient. I leave all other settings in the default.
...
dbFlow creates a folder db/_setup. Here we will find the scripts necessary to create the schema and the workspace for APEX. Additionally, a few features will be provided. These are utPLSQL, Logger, teplsql and TableAPI. For our tutorial we don't need them. Therefore I delete the files from the corresponding directory.

```bash
sources[develop]$: rm db/_setup/features/*.*
```

All files can be edited as required. dbFlow will install all existing files in the appropriate order. See: [install project](../setup/#install-project)
...
```bash
sources[develop]$: .dbFlow/setup --install
```
...
!!! danger "Important"
    ...
    Remember, dbFlow will delete the target schema on its own and create it again. If the target schema already exists, dbFlow will abort the installation. Only the `--force` option can be used to force this behavior.
...
Now you have downloaded all the files and unpacked them into your prepared dbFlow project. That means we can now tell dbFLow to install all the files. All you have to do is to build an initial version and give it the version name "install". dbFlow will then directly install the freshly built version in the current environment.
...
When you go to your APEX instance, you can log in with the default user wsadmin. When you log in for the first time, you will have to enter a new password. The initial password is always the username. Unless you have changed it before installing the setup. See (`db\_setup\workspaces\todo\create_01_user_wsadmin.sql`)
...
```bash
sources[develop]$: git checkout main
sources[main]$: git merge develop
sources[main]$: git push
```

Now we create the initial deployment for our production environment.

```bash
sources[main]$: .dbFlow/build.sh --init --version 1.0.0
```

As soon as the build of an artifact is executed by the main branch, dbFlow will ask if a tag with the corresponding version should be committed. In our case we can skip that by entering `N`dbFlow will now place the deployment artifact in the main folder in the depot.


![Screenshot depot tree](images/tutorial/todo-depot-init_1.0.0.png){ width="400px" }

> The target depot is always a directory with the same name as the branch from which the artifact is created.
...
```bash
$ .dbFlow/setup.sh --copyto "../instances/prod"
```
...
![Eintrag von xepdb1 auf xepdb2](images/tutorial/target-instance-tns.png)
...
![Eintrag von develop auf main](images/tutorial/target-instance-stage.png)
...
![Eintrag von ../_depot auf ../../_depot](images/tutorial/target-instance-depot.png)
...
```bash
instances/prod$: .dbFlow/setup.sh --install
```
...
```bash
instances/prod$: .dbFlow/apply.sh --init --version 1.0.0
```

> Since we want to do an `init` here, dbFlow will ask you if you really want to do this, since an `init` will **ALWAYS** make all target schemas empty (delete all objects). For a CI/CD environment you can prevent this by setting the environment variable DBFLOW_JENKINS="YES".
